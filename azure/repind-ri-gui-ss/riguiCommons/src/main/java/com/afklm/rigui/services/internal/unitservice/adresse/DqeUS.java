...
import com.afklm.rigui.enums.OuiNonFlagEnum;
import com.afklm.rigui.client.dqe.RNVP;
import com.afklm.rigui.client.dqe.RNVP1;
import com.afklm.rigui.client.dqe.RNVPDqeClient;
...
import com.afklm.rigui.entity.refTable.RefTableCAT_MED;
import com.afklm.rigui.entity.refTable.RefTablePAYS;
import com.afklm.rigui.entity.refTable.RefTableREF_PROVINCE;
import com.afklm.rigui.entity.refTable.RefTableREF_STA_MED;
import com.afklm.rigui.entity.reference.RefProvince;
import com.afklm.rigui.mapper.address.PostalAddressMapper;
import com.afklm.rigui.services.reference.internal.RefPaysDS;
import com.afklm.rigui.services.internal.unitservice.adresse.EDqeErrorDetails;
...
public class DqeUS {
    ...
    private void checkMandatoryAndValidity(PostalAddress pa, boolean forcage, boolean addressForFonction)
        ...
        if (StringUtils.isEmpty(pa.getScode_medium())) {
        ...
        if (StringUtils.isEmpty(pa.getSstatut_medium())) {
            throw new JrafDomainRollbackException("137"); // REF_ERREUR : MEDIUM STATUS MANDATORY
        }

        // Modification mode
        if (!StringUtils.isEmpty(pa.getSain()) && !RefTableREF_STA_MED._REF_I.equals(pa.getSstatut_medium())
        ...
        // Creation mode
        if (StringUtils.isEmpty(pa.getSain()) && !RefTableREF_STA_MED._REF_V.equals(pa.getSstatut_medium())) {
            throw new JrafDomainRollbackException("136"); // REF_ERREUR : INVALID MEDIUM STATUS
        }

        // If forcage = Yes => Num rue code postal ville not mandatory
        if (!forcage) {
            if (StringUtils.isEmpty(pa.getSno_et_rue())) {
            ...
            */
            ...
            if (StringUtils.isEmpty(pa.getSville())) {
                throw new JrafDomainRollbackException("278"); // REF_ERREUR : INVALID CITY
            }
        }

        if (!StringUtils.isEmpty(pa.getScode_province())
