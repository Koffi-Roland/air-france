...
public class Telecoms implements Serializable,Identifiable {
     ...
     */
    @Id
    @GenericGenerator(name="ISEQ_TELECOMS", strategy = "com.afklm.rigui.util.StringSequenceGenerator",
            parameters = {
    ...
    @JoinColumn(name="SGIN", nullable=true, foreignKey = @javax.persistence.ForeignKey(name = "FK_TELECOMS_SGIN_SGIN"))
    ...
    // 1 -> *
    @OneToMany()
    @JoinColumn(name="SAIN_TEL", nullable=true, foreignKey = @javax.persistence.ForeignKey(name = "FK_USAGE_MEDIUMS_TELECOMS"))
