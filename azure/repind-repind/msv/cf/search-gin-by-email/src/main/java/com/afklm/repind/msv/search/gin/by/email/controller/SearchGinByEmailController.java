...
public class SearchGinByEmailController {
    ...
    public ResponseEntity<WrapperSearchGinByEmailResponse> searchGinByEmails(
            @ApiParam(required = true, name = "email", value = "Email") @PathVariable("email") String email,
            @RequestParam(required = false, value="merge", defaultValue = "false") boolean merge) throws BusinessException {
        log.info("Searching GIN for email : {}, and merge : {}", email, merge);
        return searchGinByEmailService.search(searchGinByEmailChecker.checkSearchGinByEmail(email),merge);
    }
