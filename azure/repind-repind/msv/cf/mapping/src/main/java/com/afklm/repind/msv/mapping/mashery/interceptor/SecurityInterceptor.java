...
public class SecurityInterceptor implements HandlerInterceptor {
	...
	private String getCustomerIdFromPath(final HttpServletRequest request) {
		...
		if (pathVariables != null && !pathVariables.isEmpty() && pathVariables.containsKey(CUSTOMER_ID_PARAM)) {
			...
			if (customerId == null || "".equals(customerId) || !customerId.matches(CUSTOMER_ID_PATTERN)) {
	...
	public boolean preHandle(final HttpServletRequest request, final HttpServletResponse response, final Object handler)
		...
		if (customerIdFromPath != null && !"".equals(customerIdFromPath) && masheryUserContextHeaderExists(request)) {
