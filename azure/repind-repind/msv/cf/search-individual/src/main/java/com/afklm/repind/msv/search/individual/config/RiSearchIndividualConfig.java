...
public class RiSearchIndividualConfig implements WebMvcConfigurer {
    ...
    public RiSearchGinByEmailClient riSearchGinByEmail(){
        return new RiSearchGinByEmailClient(searchByIndividualEmailUrl+"/{email}?merge={merge}", HttpMethod.GET, SearchGinsResponse.class);
    }

    @Bean
    public RiSearchGinByContractClient riSearchGinByContract(){
        return new RiSearchGinByContractClient(searchByIndividualContractUrl+"/{contract}?merge={merge}", HttpMethod.GET, SearchGinsResponse.class);
    }

    @Bean
    public RiSearchGinByPhoneNumberClient riSearchGinByPhoneNumber(){
        return new RiSearchGinByPhoneNumberClient(searchByIndividualPhoneUrl+"/{phone}?merge={merge}", HttpMethod.GET, SearchGinsResponse.class);
    }


    @Bean
    public RiSearchGinBySocialMediaClient riSearchGinBySocialMedia(){
        return new RiSearchGinBySocialMediaClient(searchByIndividualSocialMediaUrl+"/?externalIdentifierId={externalIdentifierId}&externalIdentifierType={externalIdentifierType}&merge={merge}", HttpMethod.GET, SearchGinsResponse.class);
    }

    @Bean
    public RiSearchGinByLastnameAndFirstnameClient riSearchGinByLastnameAndFirstnameClient(){
        return new RiSearchGinByLastnameAndFirstnameClient(searchByIndividualLastnameFirstnameUrl+"/?lastname={lastname}&firstname={firstname}&merge={merge}", HttpMethod.GET, SearchGinsResponse.class);
    }
