...
class ProvideGinByIdentifierControllerTest {
    ...
    void getGinByEmail_notFound() throws Exception {
        ...
        Assert.assertEquals(BusinessError.GIN_NOT_FOUND.getRestError().getCode(), restError.getRestError().getCode());
    }
    ...
    void getGinByEmail_formatInvalid() throws Exception {
        ...
        String response = result.getResponse().getContentAsString();
        FullRestErrorMapper restError = new ObjectMapper().readValue(response, FullRestErrorMapper.class);
        Assert.assertEquals(FormatError.API_EMAIL_MISMATCH.getRestError().getCode(), restError.getRestError().getCode());
    }
    ...
    void getGinByCin_formatInvalid() throws Exception {
        ...
        String response = result.getResponse().getContentAsString();
        FullRestErrorMapper restError = new ObjectMapper().readValue(response, FullRestErrorMapper.class);
        Assert.assertEquals(FormatError.API_CIN_MISMATCH.getRestError().getCode(), restError.getRestError().getCode());
    }
}