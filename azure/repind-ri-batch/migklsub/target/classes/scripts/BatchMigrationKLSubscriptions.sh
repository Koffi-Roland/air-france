...
BATCH_PKG=com.afklm.batch.migklsub
...
do
	...
	execute ${JAVA_HOME}/bin/java  -Dbatch=BatchMigrationKLSubscriptions -server -Xmx${memory} ${JAVA17_ARG} -cp ${CLASSPATH_SIC}:${JAVA_RPD}/migklsub-0.0.1-SNAPSHOT.jar ${BATCH_PKG}.BatchMigrationKLSubscriptions $OptionsBatch
