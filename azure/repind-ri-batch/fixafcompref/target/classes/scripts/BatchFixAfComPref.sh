...
BATCH_PKG=com.afklm.batch.fixafcompref
...
# Recuperer liste des fichiers d'input et lancement du process
  ...
  OptionsBatch="$* -o $OUTPUT_DIR -p $INPUT_DIR"
	...
	execute ${JAVA_HOME}/bin/java  -Dbatch=BatchFixAfComPref -server -Xmx${memory} ${JAVA17_ARG} -cp ${CLASSPATH_SIC}:${JAVA_RPD}/fixafcompref-0.0.1-SNAPSHOT.jar ${BATCH_PKG}.BatchFixAfComPref $OptionsBatch
