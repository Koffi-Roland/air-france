...
BATCH_PKG=com.afklm.batch.individu.purge.gdpr
...
${JAVA17_ARG} -server -Xms${memory} -Xmx${memory} -cp ${CLASSPATH_SIC}:${JAVA_SIC}/individu-purge-gdpr-0.0.1-SNAPSHOT.jar ${BATCH_PKG}.BatchPurgeIndividualGDPR "$@" 2>&1 )
batch_exit_GDPR=$?
