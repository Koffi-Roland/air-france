...
elsif ($replicationNumber == 125)
{
    ...
    if (defined($startDate))
    {
        $request .= " AND (cp.modification_date >= TO_DATE('$startDate', 'YYYYMMDDHH24MISS') OR (a.DDATE_MODIFICATION IS NOT NULL AND a.DDATE_MODIFICATION >= TO_DATE('$startDate', 'YYYYMMDDHH24MISS')))";
    }
    if (defined($endDate))
    {
        $request .= " AND (cp.modification_date < TO_DATE('$endDate', 'YYYYMMDDHH24MISS') OR (a.DDATE_MODIFICATION IS NOT NULL AND a.DDATE_MODIFICATION < TO_DATE('$endDate', 'YYYYMMDDHH24MISS')))";
    }
...
elsif ($replicationNumber == 126)
{
    ...
    my $request = "SELECT ".
	  ...
	  "TO_CHAR(NVL(ml.modification_date, ml.modification_date), 'DDMMYYYY')||':#:'||ml.modification_signature||':#:'||ml.modification_site ".
        " FROM MARKET_LANGUAGE ml, COMMUNICATION_PREFERENCES cp LEFT OUTER JOIN ACCOUNT_DATA a ".
        " ON cp.SGIN = a.SGIN WHERE cp.com_pref_id = ml.com_pref_id ";
		
    if (defined($startDate))
    {
        $request .= " AND (cp.modification_date >= TO_DATE('$startDate', 'YYYYMMDDHH24MISS') OR ml.modification_date >= TO_DATE('$startDate', 'YYYYMMDDHH24MISS') or a.DDATE_MODIFICATION >= TO_DATE('$startDate', 'YYYYMMDDHH24MISS'))";
    }
    if (defined($endDate))
    {
        $request .= " AND (cp.modification_date < TO_DATE('$endDate', 'YYYYMMDDHH24MISS') OR ml.modification_date < TO_DATE('$endDate', 'YYYYMMDDHH24MISS') or a.DDATE_MODIFICATION < TO_DATE('$endDate', 'YYYYMMDDHH24MISS'))";
    }
...
#		" FROM ALERT al INNER JOIN INDIVIDUS_ALL i ON al.SGIN = i.SGIN WHERE i.STYPE <> 'W' ";
		" FROM ALERT al INNER JOIN INDIVIDUS_ALL i ON al.SGIN = i.SGIN WHERE 1 = 1 ";
...
#		" FROM ALERT_DATA ald INNER JOIN ALERT al ON ald.ALERT_ID = al.ALERT_ID INNER JOIN INDIVIDUS_ALL i on al.SGIN = i.SGIN WHERE i.STYPE <> 'W' ";
		" FROM ALERT_DATA ald INNER JOIN ALERT al ON ald.ALERT_ID = al.ALERT_ID INNER JOIN INDIVIDUS_ALL i on al.SGIN = i.SGIN WHERE 1 = 1 ";
