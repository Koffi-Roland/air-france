...
BATCH_PKG=com.afklm.batch.adrInvalidBarecode
...
execute ${JAVA_HOME}/bin/java ${JAVA17_ARG} -Dbatch=BatchAdrInvalidBarecode -server -Xmx${memory} \
                                -cp ${CLASSPATH_SIC}:${JAVA_RPD}/adrInvalidBarecode-0.0.1-SNAPSHOT.jar ${BATCH_PKG}.BatchAdrInvalidBarecode $OptionsBatch
cd $OUTPUT_DIR
