...
BATCH_PKG=com.afklm.batch.deletecontract
...
# Lancer / relancer process
	execute ${JAVA_HOME}/bin/java ${JAVA17_ARG} -Dbatch=DeleteContractsBatch -server -Xmx${memory} -cp ${CLASSPATH_SIC}:${JAVA_RPD}/deletecontract-0.0.1-SNAPSHOT.jar ${BATCH_PKG}.DeleteContractsBatch
