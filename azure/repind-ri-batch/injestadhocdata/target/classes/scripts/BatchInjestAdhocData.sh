...
BATCH_PKG=com.afklm.batch.injestadhocdata
...
do
	...
	execute ${JAVA_HOME}/bin/java ${JAVA17_ARG} -Dbatch=BatchInjestAdhocData -server -Xmx${memory} -cp ${CLASSPATH_SIC}:${JAVA_RPD}/injestadhocdata-0.0.1-SNAPSHOT.jar ${BATCH_PKG}.BatchInjestAdhocData $OptionsBatch
