...
BATCH_PKG=com.afklm.batch.individu.missing.account.data
...
cd $JAVA_SIC
execute ${JAVA_HOME}/bin/java -Dbatch=BatchAddMissingAccountData ${JAVA17_ARG} -server -Xmx${memory} -cp ${CLASSPATH_SIC}:${JAVA_SIC}/individu-missing-account-data-0.0.1-SNAPSHOT.jar ${BATCH_PKG}.BatchAddMissingAccountData $*
