...
cd $JAVA_RPD
execute ${JAVA_HOME}/bin/java ${JAVA17_ARG} -Dbatch=BatchInvalidationEmail -server -Xmx${memory} -cp ${CLASSPATH_RPD}:${JAVA_RPD}/invalidation-email-0.0.1-SNAPSHOT.jar ${BATCH_PKG}.BatchInvalidationEmail $*
