...
import com.afklm.repind.v4.provideindividualdata.type.MaskedFields;
...
@Service("paymentPreferencesDataHelperV4")
@Slf4j
public class PaymentPreferencesDataHelper {
	...
	public MaskedPaymentPreferences provideMaskedPaymentPreferences(String gin)
				...
				if (dtoList.size() > 0) {
					...
					for (FieldsDTO fieldsDTO : dtoList.get(0).getFieldsdto()) {
						...
						try{
							...
							// Non numeric fieldPref are not allowed for
							// paymentGroup CC and DC, except PCIDSS cases
							if ((result.getPaymentGroupType()
									.equalsIgnoreCase("CC") || result
									.getPaymentGroupType().equalsIgnoreCase("DC"))) {
								if (fieldCode.equalsIgnoreCase("CC_NR")) {
									...
									if (this.isNumeric(fieldPref)) {
										if (isPCIDSS.equals("Y")
											...
											pField_pci
											.setPaymentFieldCode("PCI_TOKEN");

											String pci_token = "";
											pci_token = fieldPref;

											pField_pci
											.setPaymentFieldPreferences(pci_token);
											result.getMaskedFields()
											.add(pField_pci);

											if (doWeMask(pField_pci
													.getPaymentFieldCode()) == 1) {
												pField_pci
												.setPaymentFieldPreferences(masking(
														fieldPref, false));
											} else if (doWeMask(pField_pci
													.getPaymentFieldCode()) == 2) {
												pField_pci
												.setPaymentFieldPreferences(masking(
														fieldPref, false));
											} else {
												pField_pci
												.setPaymentFieldPreferences(fieldPref);
											}
										}
							...
							if (doWeMask(pField.getPaymentFieldCode()) == 1) {
								pField.setPaymentFieldPreferences(masking(
							...
							if (!pField.getPaymentFieldCode().equalsIgnoreCase(
									"CC_NR")
									|| (pField.getPaymentFieldCode()
											.equalsIgnoreCase("CC_NR") && !isFullPCIDSS
											.equalsIgnoreCase("Y"))) {
	...
	public int computePaymentPreferencesGauge(
			MaskedPaymentPreferences paymentPref) {
		log.debug("Starting computePaymentPreferencesGauge");
		int percentage = 0;

		if (paymentPref != null && paymentPref.getAirlinePaymentPref() != null
