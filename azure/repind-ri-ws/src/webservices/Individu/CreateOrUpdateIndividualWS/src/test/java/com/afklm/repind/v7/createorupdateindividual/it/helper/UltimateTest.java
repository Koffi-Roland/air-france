...
public class UltimateTest extends UltimateHelper {

	private abstract class UltimateTesting extends UltimateHelper.Ultimate {
	...
	public void getDelegatorGinAndDelegateGin() throws Exception {
		...
		final AccountData searchDelegateAccountData = new AccountData();
		final AccountData searchDelegatorAccountData = new AccountData();
		searchDelegatorAccountData.setFbIdentifier(UltimateTest.DELEGATOR_FB);
		searchDelegateAccountData.setFbIdentifier(UltimateTest.DELEGATE_FB);

		List<AccountData> delegateAccounts = null;
		List<AccountData> delegatorAccounts = null;

		delegateAccounts = accountDataRepository.findAll(Example.of(searchDelegateAccountData));
		delegatorAccounts = accountDataRepository.findAll(Example.of(searchDelegatorAccountData));

		if (delegateAccounts == null || delegatorAccounts == null || delegatorAccounts.size() == 0
				|| delegateAccounts.size() == 0) {
			databaseInitialisation();
			return;
		}

		if (delegateAccounts.size() != 1 || delegatorAccounts.size() != 1) {
			throw new Exception("Unexpected database value");
		}

		final AccountData delegateAccount = delegateAccounts.get(0);
		final AccountData delegatorAccount = delegatorAccounts.get(0);
	...
	public final void testIsDelegateAlreadyLinkedToAUltimateCustomerCase0Link()
		...
		final List<DelegationDataDTO> limitList = new ArrayList<DelegationDataDTO>();
	...
	public final void testIsInputDelegateDoesntExistNull()
		...
		final IndividuDTO delegator = new IndividuDTO();
		final AccountDataDTO delegatorAccount = new AccountDataDTO();
		delegatorAccount.setFbIdentifier(DELEGATOR_FB);
		delegator.setAccountdatadto(delegatorAccount);
...
*/
	...
	public final void testIsUltimateFamilyTooBigCaseOverSize() throws UltimateException, NoSuchFieldException,
		...
		final List<DelegationDataDTO> overLimitList = new ArrayList<DelegationDataDTO>();
	...
	public final void testIsUltimateFamilyTooBigCaseUnderSize() throws UltimateException, NoSuchFieldException,
		...
		final List<DelegationDataDTO> underLimitList = new ArrayList<DelegationDataDTO>();
	...
	public final void testProcessAlreadyLinkedToUltimateCustomer() throws NoSuchMethodException, SecurityException,
		...
		ReflectionTestUtils.setField(ultimate, "_communicationPreferencesDS", communicationPreferencesDS);
		...
		final Method method = ultimate.getClass().getDeclaredMethod("_process", IndividuDS.class, DelegationDataDS.class, RoleDS.class, CommunicationPreferencesDS.class, VariablesRepository.class);
		method.setAccessible(true);
	...
	public final void testProcessFamilyTooBig1() throws NoSuchMethodException, SecurityException,
		...
		final Ultimate ultimate = new UltimateTesting(UltimateTest.STUB_DELEGATOR_GIN, UltimateTest.STUB_DELEGATE_GIN,
			...
			protected void _process(final IndividuDS individu, final DelegationDataDS delegationData, final RoleDS role,
					final CommunicationPreferencesDS communicationPreferences, final VariablesRepository variables) throws JrafDomainException, UltimateException {
				super._process(individu, delegationData, role, communicationPreferences, variables);
			}
		};
	...
	public final void testProcessSuccess() throws Exception {
		...
		ReflectionTestUtils.setField(_ultimate, "_delegateGin", UltimateTest._delegateSGin);
		ReflectionTestUtils.setField(_ultimate, "_originGin", UltimateTest._delegateSGin);
		final Method method = _ultimate.getClass().getDeclaredMethod("_process", IndividuDS.class, DelegationDataDS.class, RoleDS.class, CommunicationPreferencesDS.class, VariablesRepository.class);
		method.setAccessible(true);
