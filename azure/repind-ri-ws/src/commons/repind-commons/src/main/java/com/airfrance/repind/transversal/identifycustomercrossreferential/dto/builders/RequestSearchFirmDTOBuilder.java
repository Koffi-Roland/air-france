...
@Component
public class RequestSearchFirmDTOBuilder {
	...
	public RequestDTO build(com.airfrance.repind.transversal.identifycustomercrossreferential.dto.RequestDTO request)
	{
		...
		if(request != null)
		{
			...
			String searchFirmType = "A";
			if(BuildConditionDS.isTypeOfFirmConditionSet(request)) {
				searchFirmType = request.getContext().getTypeOfFirm();
			}
			identityDTO.setFirmType(searchFirmType);
			if(BuildConditionDS.isSearchLastNameIndividualConditionSet(request)) {
				identityDTO.setName(request.getSearchIdentifier().getIndividualIdentity().getLastName());
			}
			if(BuildConditionDS.isLastNameSearchTypeIndividualConditionSet(request)) {
				identityDTO.setNameSearchType(request.getSearchIdentifier().getIndividualIdentity().getFirstNameSearchType());
			}
