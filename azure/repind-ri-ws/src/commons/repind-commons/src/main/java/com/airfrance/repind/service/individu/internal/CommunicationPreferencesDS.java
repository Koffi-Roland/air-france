...
public class CommunicationPreferencesDS {
    ...
    public void update(CommunicationPreferencesDTO communicationPreferencesDTO) throws JrafDomainException {
        ...
        // chargement du bo
        communicationPreferences = communicationPreferencesRepository.getOne(communicationPreferencesDTO.getComPrefId());
    ...
    public EntityManager getEntityManager() {
        /*PROTECTED REGION ID(_knuKoNNfEeKqudTad_7FKggem ) ENABLED START*/
        return entityManager;
        /*PROTECTED REGION END*/
    }
     ...
     */
    @Transactional(rollbackFor = JrafDomainRollbackException.class, noRollbackFor = JrafDomainNoRollbackException.class)
    public void removeByGin(String sgin) throws JrafDomainException {
        /*PROTECTED REGION ID(_prjQ4NNfEeKqudTad_7FKg) ENABLED START*/
        marketLanguageRepository.removeByGin(sgin);
        communicationPreferencesRepository.deleteByGin(sgin);
        /*PROTECTED REGION END*/
    }

    /*PROTECTED REGION ID(_knuKoNNfEeKqudTad_7FKg u m) ENABLED START*/

    @Transactional(readOnly = true)
    public List<CommunicationPreferencesDTO> findCommunicationPreferences(String gin) throws JrafDomainException {

        if (StringUtils.isEmpty(gin)) {
            throw new IllegalArgumentException("Unable to find communication preferences without GIN");
        }

        List<CommunicationPreferences> communicationPreferencesList = communicationPreferencesRepository.findByGin(gin);

        if (communicationPreferencesList == null) {
            return null;
        }

        return CommunicationPreferencesTransform.bo2Dto(communicationPreferencesList);
    }
    ...
    @Transactional(rollbackFor = JrafDomainRollbackException.class, noRollbackFor = JrafDomainNoRollbackException.class)
    public int unsubscribeCommPref(String gin, String domain, String comGroupType, String comType) throws JrafDomainException {

        int unsubscribeCommPref = 0;
        int unsubscribeRelatedMarket = 0;

        if (gin != null && gin.matches("[0-9]+")) {

            // 1 - get the communication preference id and invalidate this communication preference
            // we get the last communication preference ID if several occurrences for the same couple(gin,domain,comGroupType,comType)
            CommunicationPreferencesDTO communicationPreferencesDTO = findComPrefId(gin, domain, comGroupType, comType);

            if (communicationPreferencesDTO == null) {
                throw new CommunicationPreferencesNotFoundException("SIC: COM_PREF_ID not found in COMMUNICATION PREFERENCES", gin, domain, comGroupType, comType);
            }

            if (communicationPreferencesDTO.getSubscribe() == null || !"N".equals(communicationPreferencesDTO.getSubscribe())) {

                communicationPreferencesDTO.setModificationSignature("OptOut FBSP 8");
                communicationPreferencesDTO.setModificationSite("QVI");
                CommunicationPreferences communicationPreferences = CommunicationPreferencesTransform.dto2Bo2(communicationPreferencesDTO);
                unsubscribeCommPref += communicationPreferencesRepository.unsubscribeCommPref(communicationPreferences);
                communicationPreferencesDTO = CommunicationPreferencesTransform.bo2Dto2(communicationPreferences);

                /* 2 - check for coherence between global opt-in and specific opt-ins for ML */
                if (checkOptinCoherence(communicationPreferencesDTO) == 0) {

                    unsubscribeRelatedMarket = 0;
                    /* no coherent -> set each related opt-in to 'N' if at 'Y' or 'null' */
                    Set<MarketLanguageDTO> marketLanguagesDTO = communicationPreferencesDTO.getMarketLanguageDTO();
                    for (MarketLanguageDTO marketLanguageDTO : marketLanguagesDTO) {

                        if (marketLanguageDTO.getMarketLanguageId() != null && (marketLanguageDTO.getOptIn() == null || !"N".equals(marketLanguageDTO.getOptIn()))) {

                            MarketLanguage marketLanguage = MarketLanguageTransform.dto2Bo(marketLanguageDTO);
                            marketLanguage.setModificationSignature("OptOut FBSP 8");
                            marketLanguage.setModificationSite("QVI");
                            unsubscribeRelatedMarket += marketLanguageRepository.unsubscribeMarketLanguage(marketLanguage);
                        }
                    }
                }
            }
        }
        return unsubscribeCommPref;
    }


    @Transactional(rollbackFor = JrafDomainRollbackException.class, noRollbackFor = JrafDomainNoRollbackException.class)
    public int unsubscribeCommPrefFB(String gin, String domain, String comGroupType, String comType,
                                     SignatureDTO signature) throws JrafDomainException {
        int unsubscribeCommPref = 0;
        CommunicationPreferencesDTO communicationPreferencesDTO = findComPrefId(gin, domain, comGroupType, comType);
        if (communicationPreferencesDTO != null) {

            if (communicationPreferencesDTO.getSubscribe() == null
                    || !"N".equals(communicationPreferencesDTO.getSubscribe())) {
                communicationPreferencesDTO.setModificationSignature(signature.getSignature());
                communicationPreferencesDTO.setModificationSite(signature.getSite());
                CommunicationPreferences communicationPreferences = CommunicationPreferencesTransform
                        .dto2Bo2(communicationPreferencesDTO);
                unsubscribeCommPref += communicationPreferencesRepository.unsubscribeCommPref(communicationPreferences);
                communicationPreferencesDTO = CommunicationPreferencesTransform.bo2Dto2(communicationPreferences);
                ...
                for (MarketLanguageDTO marketLanguageDTO : marketLanguagesDTO) {
                    ...
                    if (marketLanguageDTO.getMarketLanguageId() != null
                            && (marketLanguageDTO.getOptIn() == null || !"N".equals(marketLanguageDTO.getOptIn()))) {

                        MarketLanguage marketLanguage = MarketLanguageTransform.dto2Bo(marketLanguageDTO);
                        marketLanguage.setModificationSignature(signature.getSignature());
                        marketLanguage.setModificationSite(signature.getSite());
     ...
     */
    @Transactional(rollbackFor = JrafDomainRollbackException.class, noRollbackFor = JrafDomainNoRollbackException.class)
    public int unsubscribeMarketLanguage(String gin, String domain, String comGroupType, String comType, String market, String language) throws JrafDomainException {

        int unsubscribeMarket = 0;

        if (gin != null && gin.matches("[0-9]+")) {
            CommunicationPreferencesDTO comPrefDTO = findComPrefId(gin, domain, comGroupType, comType);

            if (comPrefDTO == null) {
                throw new CommunicationPreferencesNotFoundException("SIC: COM_PREF_ID not found in COMMUNICATION PREFERENCES", gin, domain, comGroupType, comType);
            }

            MarketLanguageDTO mlDTO = findMarketId(comPrefDTO.getComPrefId(), market, language);
            if (mlDTO == null) {
                throw new MarketLanguageNotFoundException("SIC: MARKET_ID not found in MARKET LANGUAGE", comPrefDTO.getComPrefId().toString(), market, language);
            }

            if (mlDTO.getOptIn() == null || !"N".equals(mlDTO.getOptIn())) {

                MarketLanguage marketLanguage = MarketLanguageTransform.dto2Bo(mlDTO);
                marketLanguage.setModificationSignature("OptOut FBSP 10");
                marketLanguage.setModificationSite("QVI");
                unsubscribeMarket = marketLanguageRepository.unsubscribeMarketLanguage(marketLanguage);

                if (checkOptinCoherence(comPrefDTO) == 0) {

                    CommunicationPreferences comPref = CommunicationPreferencesTransform.dto2Bo2(comPrefDTO);
                    comPref.setModificationSignature("OptOut FBSP 10");
                    comPref.setModificationSite("QVI");
                    @SuppressWarnings("unused")
                    int unsubscribeRelatedCommPref = communicationPreferencesRepository.unsubscribeCommPref(comPref);
                }
            }
        }
     ...
     */
    @Transactional(readOnly = true)
    public List<CommunicationPreferencesDTO> findComPrefIdByDomain(String gin, String domain) throws JrafDomainException {

        List<CommunicationPreferencesDTO> listCommunicationPreferencesDTO = new ArrayList<CommunicationPreferencesDTO>();
        List<CommunicationPreferences> listComPref = communicationPreferencesRepository.findComPrefIdByDomain(gin, domain);

        if (listComPref != null) {

            for (CommunicationPreferences comPref : listComPref) {
                if (comPref != null) {
                    CommunicationPreferencesDTO communicationPreferencesDTO = CommunicationPreferencesTransform.bo2Dto2(comPref);
                    listCommunicationPreferencesDTO.add(communicationPreferencesDTO);
                }
            }
        }
    ...
    public List<InformationsDTO> createComPrefBasedOnAPermission(PermissionsDTO permissionsDTO) throws JrafDomainException {
        ...
        for (PermissionDTO permissionDTO : permissionsDTO.getPermission()) {
            ...
            trackingPermissionsDS.tracking(permissionsDTO);
        }
