...
export class ArrayDisplayRefTableComponent implements OnInit {
  ...
  /**
   * Refresh action is used to override event action service to prevent refreshing mutiple table in the same page
   */
  @Input()
  public refreshAction$: Observable<any>;
  ...
  ngOnInit() {
    if (!this.refreshAction$) {
      this.refreshAction$ = this.eventActionService.actionExecuted$;
    }
    if (this.data) {
      this.filterValues = [];
      this.search = {};
  ...
  private initDisplayedColumns() {
    ...
    if (this.option && this.option.columns && this.option.columns.length > 0) {
      ...
      if (this.option.actionsOption.length > 0) {
        this.displayedColumns.push(new ArrayDisplayRefTableColumn('action', 'Actions', true));
      }
    } else {
