...
for myFile in $lstFiles; do
	...
	execute ${JAVA_HOME}/bin/java -Dbatch=BatchExternalIdentifier ${JAVA17_ARG} -server -Xmx${memory} -cp ${CLASSPATH_SIC}:${JAVA_SIC}/Myaccount-batch-0.1.0-SNAPSHOT.jar ${BATCH_PKG}.BatchExternalIdentifier $params
