...
if [ "$hostType" = "dev" ]
	then
	Liste_Emails_Reef="eltauber-ext@airfrance.fr"
	SQLPLUSBASE=SIC_DEV
fi
if [ "$hostType" = "rct" ]
	then
...
cd $JAVA_SIC
execute ${JAVA_HOME}/bin/java -Dbatch=BatchPurgeFirme ${JAVA17_ARG} -server -Xmx${memory} -cp ${CLASSPATH_SIC}:${JAVA_SIC}/Myaccount-batch-0.1.0-SNAPSHOT.jar ${BATCH_PKG}.BatchPurgeFirme $params
