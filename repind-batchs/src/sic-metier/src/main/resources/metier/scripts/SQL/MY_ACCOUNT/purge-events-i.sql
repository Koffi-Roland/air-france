...
/*============================================================*/
/* Traitement table TRIGGER_INSERTION                         */
/*============================================================*/

DECLARE
  CURSOR actiondate_cursor IS
	select ti.ain from sic2.trigger_insertion ti where ti.action_date < (sysdate - 1);

BEGIN
	FOR ident in actiondate_cursor
 
	LOOP
		
		DELETE FROM sic2.trigger_insertion WHERE ain = ident.ain;
		COMMIT;
