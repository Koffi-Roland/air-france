...
print ("\n\n ####### Traces d'execution de suppresion des blancs dans les emails ##### \n");
#
...
# Code Pays existe
my $resultFile = $file."_result_blanc_".getTodayDate_DDMMYYYY();
my $rejectFile = $file."_rej_blanc_".getTodayDate_DDMMYYYY();
my $traceFile = $file."_trace_blanc_".getTodayDate_DDMMYYYY();
my $nbLignes = 0;
...
while (my $line = <INPUT_FILE>) {
	$line =~ s/\r|\n//g;
  	...
  	if($line ne "") {
  		...
  		my $sEmail = $values[2];
       		print TRACE ("--> $line\n");
  		my $sRequest = "select SAIN from emails where semail='".$sEmail."' and (semail like '% %' or semail like '%'||CHR(9)||'%') and sstatut_medium='V'";
		...
		if ($selectRqt->execute()) {
  			while (my @rowGin = $selectRqt->fetchrow_array()) {
  				...
  				$requestUpdate = "update emails set semail=REPLACE(SEMAIL, ' ', '' ) , SSITE_MODIFICATION='VLB',SSIGNATURE_MODIFICATION='RetraitBlanc',DDATE_MODIFICATION=sysdate
  				where sain='".$sgin2."'";
