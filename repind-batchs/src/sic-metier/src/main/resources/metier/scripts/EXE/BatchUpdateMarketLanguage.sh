...
BATCH_PKG=com.airfrance.batch.updateMarketLanguage
...
# Lancer / relancer process
	execute ${JAVA_HOME}/bin/java  -Dbatch=BatchUpdateMarketLanguage -server -Xmx${memory} ${JAVA17_ARG} -cp ${CLASSPATH_SIC}:${JAVA_RPD}/sic-metier-root-0.0.1-SNAPSHOT.jar ${BATCH_PKG}.BatchUpdateMarketLanguage $OptionsBatch
