...
BATCH_PKG=com.airfrance.batch.detectduplicates
...
execute ${JAVA_HOME}/bin/java ${JAVA17_ARG} -Dbatch=BatchDetectDuplicates -server -Xmx${memory} -cp ${CLASSPATH_SIC}:${JAVA_TCSIC}/sic-metier-root-0.0.1-SNAPSHOT.jar ${BATCH_PKG}.BatchDetectDuplicates $OPTIONS_BATCH
