...
BATCH_PKG=com.airfrance.batch.automaticmerge
...
emailDest="mail.sic.it@airfrance.fr, tatiana.ni@klm.com"
...
execute ${JAVA_HOME}/bin/java ${JAVA17_ARG} -Djava.security.krb5.kdc=$KDCSERVER -Djava.security.krb5.realm=$REALMENV \
                                ...
                                -Dbatch=BatchAutomaticMerge -server -Xmx${memory} \
                                -cp ${CLASSPATH_SIC}:${JAVA_RPD}/sic-metier-root-0.0.1-SNAPSHOT.jar ${BATCH_PKG}.BatchAutomaticMerge $OptionsBatch
