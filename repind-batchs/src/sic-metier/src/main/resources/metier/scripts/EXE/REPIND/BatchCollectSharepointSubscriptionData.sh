...
BATCH_PKG=com.airfrance.batch.subscriptiondata
...
execute ${JAVA_HOME}/bin/java ${JAVA17_ARG} -Djava.security.krb5.kdc=$KDCSERVER -Djava.security.krb5.realm=$REALMENV \
                               ...
                               -Dbatch=BatchCollectSharepointSubscriptionData -server -Xmx${memory} \
                               -cp ${CLASSPATH_RPD}:${JAVA_RPD}/sic-metier-root-0.0.1-SNAPSHOT.jar ${BATCH_PKG}.BatchCollectSharepointSubscriptionData $*
