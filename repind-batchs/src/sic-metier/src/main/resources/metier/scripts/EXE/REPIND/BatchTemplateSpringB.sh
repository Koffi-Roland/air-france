...
BATCH_PKG=com.airfrance.batch.templatespringbatch
...
execute ${JAVA_HOME}/bin/java ${JAVA17_ARG} -Dbatch=BatchTemplateSpringB -server -Xmx${memory} -cp ${CLASSPATH_SIC}:${JAVA_RPD}/sic-metier-root-0.0.1-SNAPSHOT.jar ${BATCH_PKG}.BatchTemplateSpringB
