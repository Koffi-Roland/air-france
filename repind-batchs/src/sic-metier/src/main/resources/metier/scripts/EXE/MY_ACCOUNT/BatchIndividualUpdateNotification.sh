...
BATCH_PKG=com.airfrance.jraf.batch.event.individu 
...
execute ${JAVA_HOME}/bin/java ${JAVA17_ARG} -Dbatch=BatchIndividualUpdateNotification -server -Xmx${memory} -cp ${CLASSPATH_SIC}:${JAVA_SIC}/Myaccount-batch-0.1.0-SNAPSHOT.jar ${BATCH_PKG}.BatchIndividualUpdateNotification $*
