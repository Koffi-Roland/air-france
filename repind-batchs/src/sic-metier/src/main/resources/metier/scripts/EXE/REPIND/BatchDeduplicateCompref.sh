...
BATCH_PKG=com.airfrance.batch.deduplicatecompref
...
OptionsBatch="-G $1 -D $2 -T $3"

cd $JAVA_RPD
# Lancer / relancer process
	execute ${JAVA_HOME}/bin/java  -Dbatch=BatchDeduplicateCompref -server -Xmx${memory} ${JAVA17_ARG} -cp ${CLASSPATH_RPD}:${JAVA_RPD}/sic-metier-root-0.0.1-SNAPSHOT.jar ${BATCH_PKG}.BatchDeduplicateCompref $OptionsBatch
