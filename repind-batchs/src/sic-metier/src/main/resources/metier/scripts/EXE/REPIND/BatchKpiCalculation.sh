...
BATCH_PKG=com.airfrance.batch.kpicalculation
...
# Lancer / relancer process
	execute ${JAVA_HOME}/bin/java  -Dbatch=BatchKPICalculation -server -Xmx${memory} ${JAVA17_ARG} -cp ${CLASSPATH_RPD}:${JAVA_RPD}/sic-metier-root-0.0.1-SNAPSHOT.jar ${BATCH_PKG}.BatchKPICalculation
